...
import { Home, Search, Library, Plus, Heart, Sparkles } from "lucide-react";
    ...
    >
      ...
      <div className="glass-surface rounded-2xl p-2 flex-1 custom-scrollbar overflow-y-auto">
        <div className="px-4 py-3 flex items-center justify-between text-white/40">
          ...
          <button type="button" onClick={() => setActiveTab("Library")} title="Buka Library">
            <Plus className="size-4 hover:text-white cursor-pointer transition-colors" />
          </button>
        </div>
        <SidebarItem 
          icon={Plus} 
        ...
        <div className="mt-4 space-y-1">
          {["Dreamy Lo-fi", "Techno Pulse", "Morning Coffee", "Chill Vibes", "Coding Session"].map((name) => (
            <button
              ...
              type="button"
              onClick={() => setActiveTab("Library")}
              className="w-full text-left px-4 py-2 text-sm text-white/50 hover:text-white transition-colors truncate"
            >
              {name}
            </button>
          ))}
