...
export default function MusicCard({ song, onClick, variant = "card" }: MusicCardProps) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={() => onClick(song)}
      className="bg-[#181818] hover:bg-[#282828] active:bg-[#282828] p-3 rounded-lg cursor-pointer transition-colors group"
    >
      <div className="relative aspect-square mb-3 overflow-hidden rounded-md shadow-lg">
        ...
        />
        {/* Play overlay: visible on hover for desktop, and always tappable
            on touch devices via opacity-100 at small breakpoints handled by
            the parent's onClick (whole card is clickable either way). The
            overlay itself is shown on hover OR active/tap for feedback. */}
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
          <motion.div 
