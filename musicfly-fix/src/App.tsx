...
import { Search, Bell, Loader2, Play, Users, Clock, Plus, Heart, Sparkles, X } from "lucide-react";
...
function AppContent() {
  ...
  const { history, addToHistory, setQueue, queue } = useMusic();
  ...
  const skipPrev = () => {
    // If we're a few seconds into the song, restart it (like Spotify);
    // otherwise go back to the previously played track from history.
    if (progress > 3) {
      setProgress(0);
      return;
    }
    const prevSong = history.find(s => s.videoId !== currentSong?.videoId);
    if (prevSong) {
  ...
  const filters = ["Semua", "Musik", "Podcast"];
      ...
      <div className="flex-1 flex flex-col min-w-0 md:ml-4 w-full z-10 overflow-hidden relative">
        ...
        <header className="h-14 md:h-16 flex items-center justify-between px-4 md:px-6 shrink-0 bg-black/80 backdrop-blur-md md:glass-surface md:rounded-2xl z-20">
                ...
                {filters.map((tab) => (
                  <button 
                    ...
                    className={cn(
                      ...
                      contentFilter === tab ? "bg-white text-black" : "bg-white/10 text-white hover:bg-white/20"
                    )}
                  >
                    {tab}
                  </button>
          ...
          <div className="flex items-center gap-4">
            <button type="button" className="md:hidden" onClick={() => setActiveTab("Search")}>
            ...
            <div className="relative">
              <button
                ...
                className="hidden md:flex relative size-10 rounded-xl glass-hover glass-panel items-center justify-center"
              >
                <Bell className="size-4 text-white/60" />
                {history.length > 0 && <span className="absolute top-2 right-2 size-2 bg-white rounded-full" />}
              </button>
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    className="absolute right-0 top-12 w-72 bg-[#181818] border border-white/10 rounded-2xl shadow-2xl p-4 z-30"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-bold text-white">Baru diputar</span>
                      <button type="button" onClick={() => setShowNotifications(false)} className="text-white/40 hover:text-white">
                        <X className="size-4" />
                      </button>
                    </div>
                    {history.length > 0 ? (
                      <div className="space-y-2 max-h-64 overflow-y-auto no-scrollbar">
                        {history.slice(0, 6).map(song => (
                          <div
                            key={song.videoId}
                            onClick={() => { handlePlaySong(song); setShowNotifications(false); }}
                            className="flex items-center gap-3 p-1.5 rounded-lg hover:bg-white/5 cursor-pointer"
                          >
                            <img src={song.thumbnails[0]?.url} className="size-9 rounded object-cover" alt="" />
                            <div className="min-w-0">
                              <div className="text-xs font-semibold text-white truncate">{song.name}</div>
                              <div className="text-[10px] text-white/40 truncate">{song.artist.name}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-xs text-white/40">Belum ada riwayat pemutaran.</p>
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        <main className="flex-1 overflow-y-auto px-4 md:px-0 md:pr-2 scroll-smooth">
           {activeTab === "Library" ? (
             ...
             <GeminiTab songs={songs} onPlaySong={handlePlaySong} />
           ) : (
             <>
                       ...
                       <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                         {(songs || []).slice(0, 6).map((song) => (
                           <div 
                             ...
                             className="flex items-center gap-3 bg-white/5 hover:bg-white/10 active:bg-white/15 transition-colors rounded overflow-hidden cursor-pointer h-14 md:h-16 group"
                           >
                          ...
                          <div key={idx} className="min-w-[280px] bg-[#1a1c1a] rounded-3xl p-5 group flex flex-col h-[420px] shadow-2xl">
                             ...
                             <div className="space-y-4 mb-8">
                               {playlist.songs.map((s, i) => (
                                 <div
                                   key={i}
                                   onClick={() => handlePlaySong(s, playlist.songs)}
                                   className="flex items-center gap-4 cursor-pointer hover:bg-white/5 rounded-lg p-1 -m-1 transition-colors"
                                 >
                             ...
                             <div className="flex items-center gap-4 mt-auto">
                                ...
                                <button
                                  ...
                                  title="Rekomendasi mirip playlist ini"
                                  className="size-12 rounded-full border border-white/10 flex items-center justify-center text-white active:scale-95 transition-all hover:bg-white/10"
                                >
                                   <Sparkles className="size-6" />
                                </button>
                                <button
                                  type="button"
                                  onClick={() => playlist.songs[0] && handlePlaySong(playlist.songs[0])}
                                  title="Tambahkan ke antrean"
                                  className="size-12 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-white transition-colors ml-auto border border-white/5 active:scale-95"
                                >
...
function GeminiTab({ songs, onPlaySong }: { songs: Song[]; onPlaySong: (song: Song, context?: Song[]) => void }) {
  ...
  const handleAsk = async (e: React.FormEvent) => {
    ...
    setAsking(true);
    try {
      const results = await searchSongs(prompt);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full p-8 text-center bg-black/40 rounded-3xl animate-in fade-in zoom-in duration-500">
      ...
      <form onSubmit={handleAsk} className="w-full max-w-lg relative">
        <input 
          type="text" 
        ...
        />
        <button type="submit" disabled={asking} className="absolute right-4 top-1/2 -translate-y-1/2 size-10 rounded-xl bg-white flex items-center justify-center text-black shadow-lg disabled:opacity-50">
          {asking ? <Loader2 className="size-5 animate-spin" /> : <Sparkles className="size-5" />}
        </button>
      </form>

      {suggestions && (
        <div className="w-full max-w-3xl mt-10 grid grid-cols-2 md:grid-cols-4 gap-4 text-left">
          {suggestions.length > 0 ? suggestions.map(song => (
            <MusicCard key={song.videoId} song={song} onClick={(s) => onPlaySong(s, suggestions)} />
          )) : (
            <p className="col-span-full text-white/40 text-center">Tidak ada hasil, coba kata kunci lain.</p>
          )}
        </div>
      )}
    </div>
